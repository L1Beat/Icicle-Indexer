...
var (
	...
	lfjLBQuoter     = common.HexToAddress("0x9A93a421b74F1c5755b83dD2C211614dC419C44b")
...
func (q *blockQuoter) QuoteOut(ctx context.Context, tokenIn common.Address, _ uint8, tokenOut common.Address, _ uint8, amountIn *big.Int) (*big.Int, error) {
	...
	if q.lbQuoter != (common.Address{}) {
		for _, path := range paths {
			if out := q.lbQuote(ctx, amountIn, path, bh); out != nil && out.Cmp(best) > 0 {
				best = out
			}
		}
	}
...
// lbQuote calls LFJ LBQuoter.findBestPathFromAmountIn(route, amountIn) and reads
// the last entry of the returned Quote.amounts array.
func (q *blockQuoter) lbQuote(ctx context.Context, amountIn *big.Int, path []common.Address, bh string) *big.Int {
	res, err := q.rpc.EthCall(ctx, q.lbQuoter.Hex(), encodeFindBestPath(path, amountIn), bh)
	if err != nil {
		return nil
	}
